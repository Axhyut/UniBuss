...
const cancelSchedule = async (req, res) => {
  ...
  try {
    ...
    // Fetch all active PNRs for this schedule
    const activePnrs = await PNR.findAll({
      where: {
        scheduleId,
        status: "active",
      },
    ...
    for (const pnr of activePnrs) {
      ...
      // (Optional) Update PNR status if needed
      await pnr.update({ status: "cancelled" }, { transaction });
    }

    // Mark schedule as cancelled
    await schedule.update({ status: "cancelled" }, { transaction });
